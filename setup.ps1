# 必要なディレクトリを直接作成
New-Item -ItemType Directory -Force -Path "src/gui" | Out-Null
New-Item -ItemType Directory -Force -Path "src/hud" | Out-Null
...
function Sidebar.init(MainFrame, assets, VapeTooltip, tLabel)
    ...
    createTab("Friends", assets.friends, "rbxassetid://10747373426", 9, nil, "Configure your friend and whitelisted player settings.")
    createTab("Profiles", assets.profiles, "rbxassetid://10734898122", 10, "default", "Switch and customize your config profiles.")
    createTab("Macros", nil, "rbxassetid://10709811365", 11, nil, "Set up and bind custom macros to keys.")
    return SettingsBtn
end
return Sidebar
'@
...
local SessionInfo = {}
function SessionInfo.init(SessionFrame, SettingsBtn)
    ...
    SettingsBtn.MouseButton1Click:Connect(function()
        SessionFrame.Parent.Visible = not SessionFrame.Parent.Visible
    end)
end
return SessionInfo
'@
...
local UserInputService = game:GetService("UserInputService")
local ScreenGui = game:GetService("CoreGui"):FindFirstChild("VapeV4SidebarContainer") or game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("VapeV4SidebarContainer")

local MainContainer, MainFrame, SidebarHeader = WindowFactory.createBaseWindow(ScreenGui, "Main", UDim2.new(0, 220, 0, 403), UDim2.new(0, 15, 0.2, 0))
local VapeTooltip, tLabel = Tooltip.create(ScreenGui)
local SettingsBtn = Sidebar.init(MainFrame, assets, VapeTooltip, tLabel)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.RightControl then
        ...
        SessionContainer.Visible = visible and SessionContainer.Visible
        if TutorialContainer.Parent then
            TutorialContainer.Visible = visible
        end
        game:GetService("Lighting").VapeBlurEffect.Enabled = visible
    end
end)
...
# UTF-8 BOMなしで各ファイルを書き出す
$utf8NoBom = New-Object System.Text.UTF8Encoding $false
foreach ($pair in $files.GetEnumerator()) {
    $absolutePath = [System.IO.Path]::GetFullPath($pair.Key)
    [System.IO.File]::WriteAllText($absolutePath, $pair.Value, $utf8NoBom)
}

# cmd /c を使用して npm コマンドを安全に実行
cmd.exe /c "npm init -y"
cmd.exe /c "npm install luabundler"

# luabundlerの実行
cmd.exe /c "npx luabundler bundle src/Main.lua -p `"./src/?.lua`" -o dist/bundle.lua"